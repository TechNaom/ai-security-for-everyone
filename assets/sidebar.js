...
(function () {
  ...
  function isComplete(chapterId) {
    return !!(window.AISFEProgress && window.AISFEProgress.isComplete(chapterId));
  }

  function buildSidebarHtml(root, activeChapter) {
    const modules = window.AISFE_MODULES || [];
    let html = `<a href="${root}index.html" class="sidebar-brand">AI Security for Everyone</a>`;
