"""
Chapter 8 Project (Worked Solution): A Provenance and Integrity Checker

This is starter.py with every TODO filled in -- a real, self-contained
supply-chain defense tool: a checker that vets a synthetic set of
model/dependency artifacts against an expected manifest (Defense 1,
provenance verification), flags any artifact using an unsafe
serialization format (Defense 2, safe loading), and confirms each
artifact's publisher is on a vetted-source list -- producing a real,
...
def checksum_matches(artifact, manifest_entry):
    ...
    """
    return artifact["checksum"] == manifest_entry["expected_checksum"]
...
def format_is_safe(artifact):
    ...
    """
    return artifact["format"] in SAFE_FORMATS
...
def publisher_is_vetted(artifact, vetted_publishers):
    ...
    """
    return artifact["publisher"] in vetted_publishers
...
def vet_artifact(artifact, manifest_entry, vetted_publishers):
    ...
    return {"artifact_id": artifact.get("id", "unknown"), "findings": findings, "verdict": verdict}
