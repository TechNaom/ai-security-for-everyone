...
yarrow_health_answer = False  # Flawed: a prompt-level instruction is a
# preference the model usually follows, not an enforced boundary -- the
# lesson's own Priya incident happened with completely faithful, non-
# adversarial generation, which no prompt-level rule reliably prevents.

corrigan_next_step = "add_allowlist_check"
# Corrigan already has a PII scanner and HTML escaping in place; the
# specific, urgent, unclosed gap given the upcoming auto-fetch feature is
# the missing allow-list check on generated links -- exactly the
# downstream-API injection shape this chapter names, and the one gap that
# will be actively exploitable the moment the new feature ships.
